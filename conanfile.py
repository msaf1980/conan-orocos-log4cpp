...
class GTestConan(ConanFile):
    ...
    description = "C++ classes for flexible logging to files, syslog, IDSA and other destinations"
    #url="http://github.com/orocos-toolchain/log4cpp"
    url="http://github.com/msaf1980/orocos-log4cpp"
    license= url + "/blob/master/COPYING"
    exports = ["CMakeLists.txt", "include/*"]

    def source(self):
        zip_name = "log4cpp-%s.zip" % self.version
        durl = self.url + "/archive/master.zip"
        download(durl, zip_name)
        unzip(zip_name)
        os.system("cp -R orocos-log4cpp-%s/* ." % self.version) # hack to build as dependency
        os.unlink(zip_name)
